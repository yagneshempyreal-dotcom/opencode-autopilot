...
function apiKeyFromOpencodeConfig(cfg, provider) {
    try {
...
const KNOWN_PROVIDERS = Object.keys(PROVIDER_ENV_VARS);
function envVarCandidates(provider) {
...
export async function loadEffectiveAuth(opts) {
    ...
    const opencodeCfg = await loadOpencodeConfig(opencodePath).catch(() => ({}));
    ...
    const merged = { ...(opts?.baseAuth ?? {}), ...auth };
    const providers = new Set([
        ...KNOWN_PROVIDERS,
        ...Object.keys((opencodeCfg.provider ?? {})),
        ...Object.keys(merged),
    ]);
    for (const provider of providers) {
        ...
        if (fromEnv)
            merged[provider] = { type: "api", key: fromEnv };
    }
    return merged;
}
