...
export function heuristicScore(input) {
    ...
    if (highHits.length === 1) {
        if (promptLen > 300)
            return { tier: "high", confidence: 0.7, reason: `keyword "${highHits[0]}"` };
    ...
    if (promptLen < 80)
        return { tier: "low", confidence: 0.8, reason: "very short prompt" };
    if (promptLen < 200)
        return { tier: "low", confidence: 0.6, reason: "short prompt" };
    if (promptLen < MEDIUM_CHAR)
        return { tier: "medium", confidence: 0.55, reason: "medium prompt" };
    return { tier: "medium", confidence: 0.5, reason: "default medium" };
}
export function extractHeuristicInput(messages) {
...
function countAttachedFiles(prompt) {
