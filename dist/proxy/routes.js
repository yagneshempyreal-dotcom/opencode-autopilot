...
export async function handleChatCompletions(req, res, ctx) {
    ...
    try {
        ...
        const text = formatExhaustedReply(ctx, decision, err.message);
        respondInline(res, parsed.request.stream === true, text);
        return;
    }
...
function formatExhaustedReply(ctx, decision, errMsg) {
    ...
    if (causes.expiredOAuth.length > 0) {
        ...
        for (const p of causes.expiredOAuth)
...
function classifyCauses(attempts) {
    const out = { missingCreds: [], expiredOAuth: [], invalidCreds: [], quota: [], notFound: [] };
