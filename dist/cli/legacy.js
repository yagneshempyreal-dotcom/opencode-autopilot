import { loadConfig, saveConfig, DEFAULT_CONFIG, CONFIG_PATH } from "../config/store.js";
import { loadEffectiveAuth } from "../config/auth.js";
import { loadRecentModels } from "../bootstrap.js";
...
export async function runInit() {
    ...
    const auth = await loadEffectiveAuth();
    ...
    if (registry.models.length === 0) {
        ...
        const groups = { free: [], "cheap-paid": [], "top-paid": [] };
        for (const m of registry.models)
            groups[m.tier].push(`${m.provider}/${m.modelID}`);
        console.log("");
        for (const tier of ["free", "cheap-paid", "top-paid"]) {
            console.log(`  ${tier} (${groups[tier].length}):`);
            for (const id of groups[tier].slice(0, 8))
                console.log(`    - ${id}`);
            if (groups[tier].length > 8)
                console.log(`    … +${groups[tier].length - 8} more`);
        }
        if (registry.flagged.length > 0) {
            console.log(`\n  ⚠ ${registry.flagged.length} model(s) couldn't be auto-classified — defaulted to cheap-paid:`);
            for (const m of registry.flagged.slice(0, 5))
        ...
        ...(goal === "premium"
            ? {
                ...
                premium: {
                    ...
                    models: [
                        ...registry.models.filter((m) => m.tier === "top-paid").map((m) => `${m.provider}/${m.modelID}`),
                        ...registry.models.filter((m) => m.tier === "cheap-paid").map((m) => `${m.provider}/${m.modelID}`),
                    ].slice(0, 12),
                    freeModels: registry.models.filter((m) => m.tier === "free").map((m) => `${m.provider}/${m.modelID}`),
                },
            }
            : {}),
        tiers: {
    ...
    if (cfg.handover.enabled) {
        ...
        cfg.handover.mode = (await askChoice("  Compaction mode", ["replace", "augment"], 0));
    }
    console.log("\nPrivacy:");
    ...
    console.log(`\n✓ Saved ${CONFIG_PATH}`);
    const patchOpencode = await askYesNo("\nPatch ~/.config/opencode/opencode.json to register openauto/auto provider + plugin?", true);
    if (patchOpencode) {
        try {
            ...
            console.log(`⚠ Could not patch opencode.json: ${err.message}`);
...
async function patchOpencodeJson(port) {
    ...
    const isOurs = (p) => {
        ...
        return typeof name === "string" && (name === "opencode-openauto" ||
...
export async function runStatus() {
    ...
    if (cfg.goal === "premium" && cfg.premium) {
        console.log(`Premium pool:   ${cfg.premium.models?.length ?? 0} model(s), ${cfg.premium.retriesPerModel ?? 3} retries each`);
        console.log(`Free fallback:  ${cfg.premium.fallbackToFree ? "auto" : "prompt (router free / openauto after exhaustion)"}`);
    }
    console.log(`Allowlist:      ${cfg.allowlist?.length ? `${cfg.allowlist.length} pinned` : "(none)"}`);
...
export async function runHandovers() {
...
export async function runHandoverNow() {
...
export async function runUx(patch) {
    const cfg = await loadConfig();
...
export async function runRefresh(args) {
    ...
    if (pids.length > 0) {
        ...
        for (const p of pids)
            console.log(`  · pid=${p.pid}  tty=${p.tty}  cmd=${p.cmd}`);
    ...
    try {
        const entries = await readdir(ocCache).catch(() => []);
    ...
    try {
        const entries = await readdir(bunCache).catch(() => []);
        for (const e of entries) {
...
async function detectOpencodePids() {
    ...
    try {
        ...
        for (const line of raw.split("\n")) {
            ...
            if (!cmd)
                continue;
            if (cmd.includes("language_server") || cmd.includes("claude"))
                continue;
