...
export interface Bootstrapped {
  ctx: ProxyContext;
  autoEnabled: { get: () => boolean; set: (v: boolean) => void };
}

export async function loadRecentModels(): Promise<Array<{ providerID: string; modelID: string }>> {
  try {
...
export function pickTriageModel(models: ModelEntry[]): ModelEntry | null {
...
export async function bootstrap(opts: BootstrapOptions = {}): Promise<Bootstrapped> {
  ...
  if (typeof opts.port === "number") config.proxy.port = opts.port;
  if (opts.host) config.proxy.host = opts.host;
...
export async function warmHealth(ctx: ProxyContext): Promise<void> {
