import type { ChatMessage, Tag } from "../types.js";
...
// the bonus.
const RULES: Array<{ tag: Tag; re: RegExp }> = [
  ...
  { tag: "long-ctx", re: null as unknown as RegExp }, // computed from length below
];

const LONG_CTX_CHARS = 30_000;

export function extractTaskTags(messages: ChatMessage[]): Tag[] {
  ...
  for (const r of RULES) {
    if (!r.re) continue;
    if (r.re.test(prompt)) tags.push(r.tag);
  }
  if (total > LONG_CTX_CHARS) tags.push("long-ctx");
