...
export async function runTiers(): Promise<void> {
  const cfg = await loadConfig();
  const auth = await loadEffectiveAuth();
  const opencodeCfg = await loadOpencodeConfig();
  const recentModels = await loadRecentModels();
...
export async function runHandovers(): Promise<void> {
...
export async function runHandoverNow(): Promise<void> {
...
export async function runUx(patch: { badge: boolean }): Promise<void> {
  const cfg = await loadConfig();
