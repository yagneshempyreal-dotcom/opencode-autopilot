...
export async function formatVerifyReport(ctx: ProxyContext, noPin = false): Promise<string> {
  const models = ctx.registry.models;
  if (models.length === 0) return "(no models in registry — run `openauto init`)";
  ...
  if (report.ok.length > 0) {
    lines.push("working:");
    for (const id of report.ok) lines.push(`  ✓ ${id}`);
    lines.push("");
  }
