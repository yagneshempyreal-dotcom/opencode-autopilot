...
export async function runServe(args: string[]): Promise<void> {
  ...
  if (server.port !== requested) {
    ...
    if (requested !== 0) await saveConfig(ctx.config).catch(() => {});
  } else if (requested !== 0) {
  ...
  if (patchOpencode) {
    try {
      ...
      if (r.patched) console.log(`✓ patched opencode.json (${r.reason})`);
    } catch (err) {
      console.warn(`⚠ could not patch opencode.json: ${(err as Error).message}`);
    }
  }

  if (!skipVerify) {
    void warmHealth(ctx).catch((err) => {
      console.warn(`background verify failed: ${(err as Error).message}`);
    });
  }
