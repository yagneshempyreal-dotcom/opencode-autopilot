...
export function findModel(reg: Registry, modelRef: string): ModelEntry | null {
  ...
  if (!ref) return null;
  if (reg.byID.has(ref)) return reg.byID.get(ref) ?? null;

  // Bare model id (e.g. @gpt-5.4) — prefer exact id, then unique prefix match.
  const byModelId = reg.models.filter((m) => m.modelID === ref);
  if (byModelId.length === 1) return byModelId[0] ?? null;
  if (byModelId.length > 1) {
    const openai = byModelId.filter((m) => m.provider === "openai");
    if (openai.length === 1) return openai[0] ?? null;
    return byModelId[0] ?? null;
  }

  const prefixMatches = reg.models.filter(
    (m) => m.modelID.startsWith(ref) || ref.startsWith(m.modelID),
  );
