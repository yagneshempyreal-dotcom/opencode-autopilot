...
export async function loadHealth(path: string = healthPath()): Promise<HealthStore> {
  try {
    ...
    logger.warn("health.json corrupted — starting fresh", { err: (err as Error).message });
    return emptyStore();
  }
}

export async function saveHealth(store: HealthStore, path: string = healthPath()): Promise<void> {
...
export function key(provider: string, modelID: string): string {
  return `${provider}/${modelID}`;
}

export function markOk(store: HealthStore, k: string, latencyMs?: number): void {
  const now = Date.now();
  store.records[k] = {
    status: "ok",
    lastChecked: now,
...
export function markDown(store: HealthStore, k: string, reason: string): void {
...
export function isHealthy(store: HealthStore, k: string, now: number = Date.now()): boolean {
...
export interface VerifyReport {
  total: number;
  ok: string[];
  down: Array<{ id: string; status: number; error?: string }>;
  durationMs: number;
}
